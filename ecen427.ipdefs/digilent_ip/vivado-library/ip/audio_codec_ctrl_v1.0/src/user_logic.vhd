...
architecture IMP of user_logic is
  ...
  signal fifo_tx_l_wr_en : std_logic;
  signal fifo_tx_r_wr_en : std_logic;
  signal tx_irq_l : std_logic;
  signal tx_irq_r : std_logic;
  
  signal tx_datacount_l : std_logic_vector(9 downto 0);
...
begin
  ...
  begin
    if (rising_edge(Bus2IP_Clk)) then
  	...
  	Inst_iis_ser: iis_ser PORT MAP(
		...
		SDATA => SDATA_O,
		EN => en,
		LDATA => fifo_tx_l_dout,
		RDATA => fifo_tx_r_dout
	);
  
  
  Inst_fifo_tx_l : fifo PORT MAP (
       clk => Bus2IP_Clk,
        srst => '0',
        din => DataTx_L(23 downto 0),
        wr_en => fifo_tx_l_wr_en,
        rd_en => data_rdy,
        dout => fifo_tx_l_dout,
        full => open,
        empty => open,
        data_count => tx_datacount_l,
        prog_empty => tx_irq_l
  );
  
  Inst_fifo_tx_r : fifo PORT MAP (
       clk => Bus2IP_Clk,
        srst => '0',
        din => DataTx_R(23 downto 0),
        wr_en => fifo_tx_r_wr_en,
        rd_en => data_rdy,
        dout => fifo_tx_r_dout,
        full => open,
        empty => open,
  ...
  begin
    if (rising_edge(Bus2IP_Clk)) then
        if (slv_reg_write_sel = "00100") then
            fifo_tx_l_wr_en <= '1';
        else 
            fifo_tx_l_wr_en <= '0';
        end if;
        
        if (slv_reg_write_sel = "00010") then
            fifo_tx_r_wr_en <= '1';
        else
            fifo_tx_r_wr_en <= '0';
        end if;
  ...
  tx_irq <= (tx_irq_l or tx_irq_r) and intr_enable;
  ...
  begin
    ...
    case slv_reg_read_sel is
      ...
      when "00001" => slv_ip2bus_data <= "0000000000" & data_rdy_bit & tx_datacount_l & tx_datacount_r & intr_enable;
